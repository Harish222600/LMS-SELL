...
const Img = ({ src, className, alt, width, height }) => {
    ...
    return (
        <div className={`relative overflow-hidden rounded-lg border border-academic-slate-200 ${className}`}>
            <LazyLoadImage
                ...
                style={{
                    minHeight: '100%',
                    background: 'linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%)', // Academic gradient background
                    transition: 'all 0.3s ease-in-out'
                }}
                onError={(e) => {
                    console.error('❌ Image failed to load:', src);
                    console.error('Error details:', e);
                    // Set a fallback background for failed images
                    e.target.style.background = 'linear-gradient(135deg, #f1f5f9 0%, #cbd5e1 100%)';
                }}
                onLoad={() => {
            ...
            {/* Academic-themed loading overlay */}
            <div className="absolute inset-0 bg-gradient-to-br from-academic-cream-50 to-academic-slate-100 opacity-0 transition-opacity duration-300 flex items-center justify-center">
                <div className="w-8 h-8 border-2 border-academic-gold-500 border-t-transparent rounded-full animate-spin"></div>
            </div>
        </div>
    )
}
